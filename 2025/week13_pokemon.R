...
pokemon_df <- raw_data$pokemon_df
      ...
      #  select(where(is.numeric)) |> 
        select(
           ...
           #    -generation_id,
               ...
               -color_1,-color_2,-color_f, -url_icon,-url_image) |> 
        mutate(height=log10(height),
               weight=log10(weight)) |> 
        ggpairs()
...
pokemon_df |>
        filter(!is.na(egg_group_2)) |> 
        ggplot(aes(x=egg_group_1, y=egg_group_2))+
...
pokemon_df |> 
        ...
        ggplot(
                aes(
                        #x=species_id,y=base_experience, colour = generation_id
                        x=type_1,y=value, fill = type_1
                        )
                )+
        geom_boxplot()+
        theme_bw()+
        ...
        xlab("Pokemon Type")+ylab("")+
        facet_wrap(~name, scales="free")

images <- pokemon_df |> 
        filter(pokemon %in% c(
                              "machop","mankey","hitmonchan",
                              "klinklang","steelix","dialga",
                              "poliwag","lotad","staryu")) |> 
        select(type_1, pokemon, url_image) |>
        arrange(type_1) |> 
        mutate(x=c(87,91,96,
                   150,160,145,
                   92,102,117),
